...
export function TeacherSummaryEditor({ initial, onSave, onClose }) {
  ...
  const save=()=>{
    const secoes=sections.map(s=>({titulo:s.titulo.trim(),explicacao:s.explicacao.trim(),exemplo:s.exemplo.trim()})).filter(s=>s.titulo&&s.explicacao);
    const atividade=questions.map(q=>({q:q.q.trim(),opts:q.opts.map(o=>o.trim()),correct:Number(q.correct)})).filter(q=>q.q&&q.opts.every(Boolean));
    if(!intro.trim()||!secoes.length){setError("Escreva a introdução e pelo menos uma seção completa.");return;}
    if(questions.some(q=>q.q.trim()||q.opts.some(Boolean))&&atividade.length!==questions.length){setError("Complete as quatro alternativas ou remova a pergunta incompleta.");return;}
    onSave({intro:intro.trim(),secoes,dica:tip.trim(),atividade,manual:true});
  };
  return <div style={{position:"fixed",inset:0,zIndex:1250,background:"#0b0614dd",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div className="pop" style={{width:"min(760px,100%)",maxHeight:"90vh",overflowY:"auto",background:"linear-gradient(180deg,#231636,#1a1029)",border:"1px solid #c084fc66",borderRadius:20,padding:20}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:12}}><div><h2 style={{margin:0,color:"#f0e9fb",fontSize:19}}>✍️ Escrever resumo e atividade</h2><p style={{color:"#a99ac9",fontSize:12.5,margin:"4px 0 14px"}}>As seções ficam organizadas no mesmo formato do resumo do Nyx.</p></div><button onClick={onClose} style={{background:"none",border:0,color:"#a99ac9",fontSize:22,cursor:"pointer"}}>✕</button></div>
    <textarea value={intro} onChange={e=>setIntro(e.target.value)} rows={3} placeholder="Introdução: o que a turma aprendeu?" style={{...field,resize:"vertical"}}/>
