...
export function TeacherNyxAlertBubble({students=[],checkins={},helpNotice="",onViewStudent}) {
  ...
  const alerts=useMemo(()=>{const now=Date.now(),items=[];students.forEach(student=>{const key=`${student.shift||"sem-turno"}:${student.name}`;
    ...
    Object.entries(student.justifications||{}).forEach(([date,justification])=>{if(justification?.status==="pending")items.push({id:`absence:${key}:${date}:${justification.at||0}`,type:"absence",student,at:justification.at||0,title:"Justificou uma falta",text:`${student.name} enviou uma justificativa para ${date.split("-").reverse().join("/")}: ${String(justification.text||"").slice(0,120)}`});});});
  ...
  const colorFor=type=>type==="help"?"#f87171":type==="absence"?"#fbbf24":"#c084fc";
  const iconFor=type=>type==="help"?"✋":type==="absence"?"📅":"💜";
  return <aside style={{position:"fixed",right:82,bottom:18,zIndex:1260}}>
    {open&&<div role="dialog" aria-label="Avisos do Nyx" style={{width:"min(360px,calc(100vw - 28px))",maxHeight:"62vh",overflowY:"auto",marginBottom:10,background:"linear-gradient(180deg,#24143a,#140d22)",border:"1px solid #c084fc",borderRadius:18,padding:14,boxShadow:"0 20px 55px #000a"}}>
      ...
      {alerts.map(alert=><article key={alert.id} style={{background:seen[alert.id]?"#ffffff06":`${colorFor(alert.type)}12`,border:`1px solid ${colorFor(alert.type)}66`,borderRadius:12,padding:10,marginTop:8,opacity:seen[alert.id]?.7:1}}>
        <strong style={{color:colorFor(alert.type),fontSize:12.5}}>{iconFor(alert.type)} {alert.title}</strong><p style={{color:"#e9ddf6",fontSize:12,margin:"4px 0"}}>{alert.text}</p><small style={{color:"#776798"}}>{new Date(alert.at).toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"short"})}</small>
