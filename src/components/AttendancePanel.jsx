...
export function AttendancePanel({ students, shiftFilter, shiftLabel, onSet }) {
  ...
  return <section data-tour-prof="attendance-body" style={{maxWidth:1180, margin:'18px auto', padding:20, color:'#eee', background:'#171026', borderRadius:16}}>
    ...
    <label>Dia da chamada <input aria-label="Dia da chamada" type="date" value={day} max={todayKey()} onChange={e => e.target.value && setDay(e.target.value)} disabled={!!busy}/></label>
      ...
      <tbody>{shown.map(s => {
        ...
        return <tr key={`${s.shift}:${s.name}`}>
          ...
          <td><div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
            <button disabled={!!busy} onClick={() => save(s, 'present')}>Dar presença</button>
            <button disabled={!!busy} onClick={() => save(s, 'absent')}>Tirar presença</button>
            {manual && <button disabled={!!busy} onClick={() => save(s, 'auto')}>Voltar ao automático</button>}
          </div></td>
        </tr>;
      })}</tbody>
    </table></div>
