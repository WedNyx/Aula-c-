...
export const TEST_SHIFT = { id:"teste", label:"Teste", emoji:"🧪" };
...
export const LANG_SHIFT = { id:"linguagens", label:"Linguagens", emoji:"🌐" };
export const LANG_SHIFT_PASSWORD = "MultiLang2026";
export const shiftMeta  = id => SHIFTS.find(s=>s.id===id) || (id===TEST_SHIFT.id ? TEST_SHIFT : id===LANG_SHIFT.id ? LANG_SHIFT : { id:id||"", label:"Sem turno", emoji:"" });
export const shiftLabel = id => { const m = shiftMeta(id); return `${m.emoji} ${m.label}`.trim(); };
export const isSameDayTs = (ts) => !!ts && new Date(ts).toDateString() === new Date().toDateString();

// conteúdo do dia por turno — aceita o formato antigo (string única) como legado
export function contentNameFor(value, shift) {
  if (!value) return "";
  if (typeof value === "string") return value; // legado: mesmo texto pros dois turnos
  return value[shift] || "";
}
export function withContentName(contentNames, date, shift, title) {
