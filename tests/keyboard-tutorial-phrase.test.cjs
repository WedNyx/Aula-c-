// Tutorial de teclado (Fase 6): depois de acertar uma tecla, a Nyx gera uma frasezinha (aqui
// mockada) pro aluno digitar de verdade antes de avançar — a comparação ignora quantas linhas o
// texto quebra e ignora o que estiver DENTRO de aspas (só a estrutura em volta precisa bater).
// No Modo Guiado essa etapa é pulada — avança direto depois de acertar a tecla, sem digitar frase.
...
(async () => {
  ...
  // ── 1) fluxo normal: acerta a tecla → aparece a frase → digitar igual (com aspas diferentes) avança ──
  {
    ...
    check('A frase mockada aparece na tela', (await page.locator('text=/Console.WriteLine/').count()) > 0);
    ...
    await textarea.type('Console.WriteLine("outra coisa qualquer");');
    ...
    await textarea2.type('isso aqui não bate com a estrutura esperada');
    await page.waitForTimeout(400);
