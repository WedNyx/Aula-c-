...
const checks = [
  ...
  ["órbita e cauda prismáticas não vazam para todas as skins", prisma.includes("keepsPrismaOrbit") && prisma.includes("keepsPrismaTail")],
  ["cada aparência possui núcleo temático", ["orbita","guardiao","aurora","lua-nova","mare","constelacao"].every(id => prisma.includes(`data-skin-core=\"${id}\"`))],
  ["aurora usa cortinas luminosas próprias", prisma.includes('id={uid + "aurora"}') && prisma.includes('data-skin-head="aurora"')],
  ["lua nova combina eclipse, crescentes e carapaça escura", prisma.includes('data-skin-head="lua-nova"') && prisma.includes('data-skin-core="lua-nova"')],
  ["constelação possui asas e mapa estelar", prisma.includes('data-skin-head="constelacao"') && prisma.includes('data-skin-core="constelacao"')],
  ["Eclipse tem prioridade automática", app.includes('const nyxUnavailable = aiDown || connected === false;') && app.includes('nyxUnavailable ? "skinEclipse"')],
