...
const checks = [
  ...
  ["os alvos novos existem no painel e na barra lateral", ["perfil", "jornada", "conquistas", "ranking", "games", "santuario", "conhecimento", "desafio-livre"].every(x => app.includes(`data-tour=\"${x}\"`) || app.includes(`tour:\"${x}\"`)) && sidebar.includes("data-tour={item.tour}")],
  ...
  ["o limite por categoria também protege compras novas", shop.includes("SLOT_LABELS[item.slot]||item.slot") && shop.includes("Tire um deles antes de comprar outro.")],
