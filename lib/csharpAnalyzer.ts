...
function generateSummary(code: string, concepts: string[]): string {
  ...
  if (hasOutput) summary += '4. **`Console.WriteLine()`** — exibe texto no terminal/console\n'
  if (hasInput) summary += '5. **`Console.ReadLine()`** — lê o que o usuário digitar no teclado\n'

  return summary
}

function generateQuestions(code: string, concepts: string[]): Array<{id: string, text: string, options: string[], correctIndex: number}> {
  const allQuestions = [
    { text: 'Qual é o comando usado para exibir texto no console em C#?', options: ['print()', 'Console.WriteLine()', 'echo()', 'System.out.println()'], correctIndex: 1 },
    { text: 'O que significa "static void Main(string[] args)"?', options: ['É uma variável especial', 'É o método principal onde o programa começa', 'É uma classe de sistema', 'É um comentário do código'], correctIndex: 1 },
    { text: 'Para que serve o "using System;" no início do código?', options: ['Para criar variáveis', 'Para fechar o programa', 'Para importar a biblioteca padrão do C#', 'Para definir o nome do projeto'], correctIndex: 2 },
    { text: 'Como declaramos uma variável do tipo inteiro em C#?', options: ['var numero = 10;', 'integer numero = 10;', 'int numero = 10;', 'num numero = 10;'], correctIndex: 2 },
    { text: 'Qual é a sintaxe correta de um if em C#?', options: ['if x > 5 then', 'if (x > 5) {', 'if x > 5:', 'if[x > 5]'], correctIndex: 1 },
    { text: 'Como lemos dados do teclado em C#?', options: ['input()', 'Console.ReadLine()', 'scanf()', 'read()'], correctIndex: 1 },
    { text: 'O que é uma "classe" em C#?', options: ['Um tipo especial de variável', 'Uma função matemática', 'Um modelo para criar objetos com propriedades e métodos', 'Um comando para mostrar texto'], correctIndex: 2 },
    { text: 'Como criamos um laço que repete 5 vezes em C#?', options: ['repeat(5) {}', 'loop 5 times {}', 'for (int i = 0; i < 5; i++) {}', 'while count 5 {}'], correctIndex: 2 },
    { text: 'Qual é a diferença entre "int" e "double" em C#?', options: ['Não há diferença', 'int é para números inteiros, double é para números decimais', 'int é para texto, double é para números', 'double é mais rápido que int'], correctIndex: 1 },
    { text: 'O que significa a palavra-chave "string" em C#?', options: ['Um número decimal', 'Um valor verdadeiro ou falso', 'Uma sequência de caracteres (texto)', 'Um número inteiro'], correctIndex: 2 },
  ]
...
function generateStudentFeedback(concepts: string[], errors: CodeError[]): string {
  ...
  if (errors.length === 0 && concepts.length > 0) {
    ...
    feedback += '**O que fazer a seguir:**\n- Tente adicionar mais funcionalidades ao seu código\n- Experimente usar os conceitos em situações diferentes\n- Ajude um colega que está tendo dificuldade\n'
    ...
    feedback += '\n**Dica:** Não desista! Todo programador comete erros no início.\nLeia as mensagens do robô assistente com atenção.\n'
  } else {
    feedback += '### Bom início! 👍\n\nVocê criou um programa básico. Tente expandir com mais funcionalidades!\n\n**Sugestões:**\n- Adicione uma variável para guardar um valor\n- Peça para o usuário digitar algo com Console.ReadLine()\n- Use uma condição if para tomar uma decisão\n'
  }
  return feedback
}
...
export function analyzeCode(code: string, studentName = 'Aluno'): AnalysisResult {
  ...
  lines.forEach((line, idx) => {
    ...
    if (/static\s+void\s+Main/i.test(trimmed)) {
      if (!/static\s+void\s+Main/.test(trimmed)) {
        errors.push({ line: lineNum, type: 'error', message: `Linha ${lineNum}: "Main" está com letra errada.`, fix: 'Use "Main" com M maiúsculo: static void Main(string[] args)' })
      } else { hasMain = true }
    }
    if (/Console\.writeline\s*\(/i.test(trimmed) && !/Console\.WriteLine\s*\(/.test(trimmed)) {
      errors.push({ line: lineNum, type: 'error', message: `Linha ${lineNum}: "Console.WriteLine" está escrito errado.`, fix: 'Escreva exatamente: Console.WriteLine() — com W e L maiúsculos.' })
    }
    if (/console\./i.test(trimmed) && !/Console\./.test(trimmed)) {
      errors.push({ line: lineNum, type: 'error', message: `Linha ${lineNum}: "Console" deve começar com C maiúsculo.`, fix: 'Corrija para: Console.WriteLine() ou Console.ReadLine()' })
    }
    ...
    if (trimmed.endsWith(')') && !trimmed.endsWith('=>') && !trimmed.startsWith('//') &&
      ...
      if (/\w+\s*\(.*\)$/.test(trimmed) && !trimmed.startsWith('using')) {
        ...
        if (!isControlFlow && !trimmed.endsWith(';')) {
          warnings.push({ line: lineNum, type: 'warning', message: `Linha ${lineNum}: Possível falta de ponto e vírgula (;) no final da linha.`, fix: 'Adicione ; ao final: ' + trimmed + ';' })
        }
      }
    }
  })
  ...
  if (bracketCount > 0) {
    errors.push({ line: 0, type: 'error', message: `Faltam ${bracketCount} chave(s) de fechamento "}"`, fix: `Adicione ${bracketCount} chave(s) "}" ao final do código para fechar os blocos abertos.` })
  } else if (bracketCount < 0) {
    errors.push({ line: 0, type: 'error', message: `Há ${Math.abs(bracketCount)} chave(s) "}" a mais no código`, fix: `Remova ${Math.abs(bracketCount)} chave(s) "}" desnecessárias.` })
  }
  ...
  if (!code.trim() || code.trim() === '// Novo arquivo C#') {
    ...
    robotMessage = `✅ Parabéns! Seu código está correto! Você usou ${concepts.length} conceito(s) de C#: ${concepts.slice(0, 2).join(', ')}${concepts.length > 2 ? ' e mais!' : '.'} Continue assim!`
  } else if (!isValid) {
  ...
  return { isValid, errors, warnings, feedback, robotMessage, concepts }
}
...
export function generateFullReport(code: string, studentName: string) {
  ...
  const questions = generateQuestions(code, analysis.concepts)
  const teacherFeedback = generateTeacherFeedback(studentName, analysis.concepts, analysis.errors)
  return { analysis, summary, questions, teacherFeedback }
}
