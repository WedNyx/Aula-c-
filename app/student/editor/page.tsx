...
export default function StudentEditor() {
  ...
  const handleRename = (fileId: string) => {
    ...
    const name = renameValue.trim().endsWith('.cs') ? renameValue.trim() : renameValue.trim() + '.cs'
    ...
    <div className="h-screen flex flex-col" style={{ background: '#0d1117' }}>
      <div className="flex items-center justify-between px-4 py-2 border-b flex-shrink-0" style={{ background: '#161b22', borderColor: '#30363d' }}>
        <div className="flex items-center gap-3">
          <div className="p-1.5 rounded" style={{ background: '#23863620' }}>
            <FileCode size={18} color="#238636" />
          </div>
          <span className="font-semibold text-sm" style={{ color: '#e6edf3' }}>Aula de C#</span>
          <span className="text-xs px-2 py-0.5 rounded" style={{ background: '#1f6feb20', color: '#1f6feb' }}>
            {currentStudent.fullName}
          </span>
          <span className="text-xs px-2 py-0.5 rounded capitalize" style={{ background: '#30363d', color: '#8b949e' }}>
            {currentStudent.classType}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => { useAppStore.getState().setCurrentStudent(null); router.push('/') }}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-52 flex-shrink-0 border-r flex flex-col" style={{ background: '#161b22', borderColor: '#30363d' }}>
          <div className="p-2 border-b flex items-center justify-between" style={{ borderColor: '#30363d' }}>
            ...
            <button onClick={() => setShowNewFile(true)} title="Novo arquivo" className="p-1 rounded hover:bg-gray-700" style={{ color: '#8b949e' }}>
              <FilePlus size={14} />
            </button>
          </div>
          ...
          {showNewFile && (
            <div className="p-2 border-b" style={{ borderColor: '#30363d' }}>
              <input
                ...
                className="w-full p-1.5 text-xs rounded outline-none"
          ...
          <div className="flex-1 overflow-y-auto p-1">
            {studentFiles.map(file => (
              <div
                key={file.id}
                className="group flex items-center gap-1 rounded px-2 py-1.5 cursor-pointer"
              ...
              >
                ...
                {renamingId === file.id ? (
                  <input
                    ...
                    style={{ color: '#e6edf3', borderColor: '#1f6feb' }}
                  />
                ) : (
                  <span className="flex-1 text-xs truncate" style={{ color: '#e6edf3' }}>{file.name}</span>
                )}
                <div className="hidden group-hover:flex items-center gap-0.5">
                  <button onClick={e => { e.stopPropagation(); handleStartRename(file.id, file.name) }} className="p-0.5 rounded hover:bg-gray-600" style={{ color: '#8b949e' }}>
                    <Pencil size={10} />
                  </button>
                  {studentFiles.length > 1 && (
                    <button onClick={e => { e.stopPropagation(); handleDelete(file.id) }} className="p-0.5 rounded hover:bg-gray-600" style={{ color: '#f85149' }}>
                      <Trash2 size={10} />
                    </button>
                  )}
                </div>
            ...
            <div className="h-40 flex-shrink-0 border-t flex flex-col" style={{ background: '#0d1117', borderColor: '#30363d' }}>
              <div className="flex items-center justify-between px-3 py-1 border-b" style={{ background: '#161b22', borderColor: '#30363d' }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium" style={{ color: '#8b949e' }}>Terminal</span>
                </div>
              ...
              <div ref={terminalRef} className="flex-1 overflow-y-auto p-3 font-mono text-xs" style={{ color: '#3fb950' }}>
                {terminalOutput.map((line, i) => (
                  <div key={i} style={{ color: line.startsWith('❌') ? '#f85149' : '#3fb950' }}>{line || ' '}</div>
                ))}
              </div>
            </div>
          )}
        ...
        {showRobot && (
          <div className="w-72 flex-shrink-0 border-l flex flex-col" style={{ background: '#161b22', borderColor: '#30363d' }}>
            <div className="p-3 border-b flex items-center justify-between" style={{ borderColor: '#30363d' }}>
              <div className="flex items-center gap-2">
                <Bot size={16} color="#238636" />
                <span className="text-sm font-semibold" style={{ color: '#e6edf3' }}>Robô Assistente</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              <div className="p-3 rounded-lg" style={{ background: '#0d1117', border: '1px solid #30363d' }}>
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs leading-relaxed" style={{ color: '#e6edf3' }}>
                    {analysis?.robotMessage || '👋 Olá! Escreva seu código e eu vou verificar se está correto!'}
                  </p>
                </div>
              </div>
              ...
              {analysis && (
                <div
                  ...
                  style={{
                    background: analysis.isValid ? '#23863620' : '#f8514920',
                    border: `1px solid ${analysis.isValid ? '#238636' : '#f85149'}40`,
                    color: analysis.isValid ? '#3fb950' : '#f85149',
                  }}
                >
                  {analysis.isValid
                    ? <><CheckCircle size={14} /> Código correto!</>
                    : <><XCircle size={14} /> {analysis.errors.length} erro(s) encontrado(s)</>
                  }
                </div>
              )}
                  ...
                  {analysis.warnings.map((w, i) => (
                    <div key={i} className="p-2 rounded-lg text-xs" style={{ background: '#d2992210', border: '1px solid #d2992230' }}>
                      <div className="flex items-start gap-1">
                        <AlertCircle size={10} color="#d29922" className="mt-0.5 flex-shrink-0" />
                        <p style={{ color: '#d29922' }}>{w.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {analysis.concepts.map((c, i) => (
                      <span key={i} className="text-xs px-2 py-0.5 rounded" style={{ background: '#1f6feb20', color: '#1f6feb' }}>{c}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="pt-2 border-t" style={{ borderColor: '#30363d' }}>
                ...
                <div className="space-y-1.5 text-xs" style={{ color: '#8b949e' }}>
                  <div className="flex items-center gap-2">
                    <span className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: '#238636', color: 'white' }}>▶ Executar</span>
                    <span>Roda o código</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: '#1f6feb', color: 'white' }}>💾 Salvar</span>
                    <span>Salva e vai ao resumo</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: '#30363d', color: '#e6edf3' }}>📄+</span>
      ...
      <div className="flex items-center gap-2 px-4 py-1 border-t" style={{ background: '#161b22', borderColor: '#30363d' }}>
        <button
          ...
          style={{ background: showTerminal ? '#30363d' : 'transparent', color: '#8b949e' }}
        >
        ...
        <button
          ...
          className="text-xs flex items-center gap-1 px-2 py-0.5 rounded"
          style={{ background: showRobot ? '#30363d' : 'transparent', color: '#8b949e' }}
        >
          <Bot size={12} /> Robô
        </button>
      </div>
