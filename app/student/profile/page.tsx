...
export default function StudentProfile() {
  ...
  const handleCreate = () => {
    ...
    const existing = students.find(s =>
      s.fullName.toLowerCase() === name.trim().toLowerCase() &&
        ...
        {mode === 'create' && (
          <div>
            <div className="flex items-center gap-3 mb-6">
              <button onClick={() => setMode('choose')} style={{ color: '#8b949e' }}>
                <ArrowLeft size={20} />
              </button>
            ...
            <div className="p-6 rounded-xl border" style={{ background: '#161b22', borderColor: '#30363d' }}>
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full p-3 rounded-lg text-sm outline-none focus:ring-2"
                  style={{
                    background: '#0d1117',
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(['matutino', 'vespertino'] as ClassType[]).map(type => (
                    <button
                      ...
                      style={{
                        background: classType === type ? '#1f6feb20' : '#0d1117',
                        borderColor: classType === type ? '#1f6feb' : '#30363d',
                        color: classType === type ? '#1f6feb' : '#8b949e',
                      }}
                    >
              ...
              {error && (
                <div className="mb-4 p-3 rounded-lg text-sm" style={{ background: '#f8514920', color: '#f85149', border: '1px solid #f8514940' }}>
                  {error}
                </div>
              )}
              ...
              <button
                ...
                className="w-full p-3 rounded-lg font-medium transition-all hover:opacity-90"
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['matutino', 'vespertino'] as ClassType[]).map(type => (
                  <button
                    ...
                    style={{
                      background: filterClass === type ? '#1f6feb20' : '#161b22',
                      borderColor: filterClass === type ? '#1f6feb' : '#30363d',
                      color: filterClass === type ? '#1f6feb' : '#8b949e',
                    }}
                  >
                    {type === 'matutino' ? '🌅 Matutino' : '🌇 Vespertino'}
                  </button>
                  ...
                  .map(student => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-full" style={{ background: '#23863620' }}>
                          <UserCircle size={20} color="#238636" />
                        </div>
                        <div>
                          <p className="font-medium" style={{ color: '#e6edf3' }}>{student.fullName}</p>
                          <p className="text-xs" style={{ color: '#8b949e' }}>
                            Último acesso: {new Date(student.lastActiveAt).toLocaleDateString('pt-BR')}
                          </p>
                        </div>
                      </div>
                    </button>
