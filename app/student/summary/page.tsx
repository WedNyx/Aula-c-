...
function SummaryContent() {
  ...
  useEffect(() => {
    ...
    if (!session.summary) {
      ...
      updateSession(session.id, {
        ...
        studentFeedback: r.analysis.feedback,
        teacherFeedback: r.teacherFeedback,
        status: 'summary',
      })
    } else {
      setReport({
        analysis: { isValid: true, errors: [], warnings: [], feedback: session.studentFeedback, robotMessage: '', concepts: [] },
  ...
  const score = submitted
    ? report?.questions.filter((q) => answers[q.id] === q.correctIndex).length || 0
    : 0
    ...
    <div className="min-h-screen" style={{ background: '#0d1117' }}>
      <div className="border-b px-6 py-3 flex items-center justify-between" style={{ background: '#161b22', borderColor: '#30363d' }}>
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/student/editor')} style={{ color: '#8b949e' }}>
            <ChevronLeft size={20} />
          </button>
        ...
        <div className="flex gap-2">
          {(['summary', 'questions', 'feedback'] as const).map((p, i) => (
            <button
              key={p}
              onClick={() => phase !== 'feedback' && setPhase(p)}
              className="text-xs px-3 py-1 rounded-full"
              style={{ background: phase === p ? '#1f6feb' : '#21262d', color: phase === p ? 'white' : '#8b949e' }}
            >
      ...
      <div className="max-w-3xl mx-auto p-6">
        {phase === 'summary' && (
          <div className="space-y-4">
            ...
            <button onClick={() => setPhase('questions')} className="w-full flex items-center justify-center gap-2 p-3 rounded-xl font-medium transition-all hover:opacity-90" style={{ background: '#238636', color: 'white' }}>
              Ir para as Atividades <ArrowRight size={16} />
            </button>
          </div>
        )}
        ...
        {phase === 'questions' && (
          <div className="space-y-4">
            ...
            {report.questions.map((q, i) => (
              <div key={q.id} className="p-4 rounded-xl border" style={{ background: '#161b22', borderColor: '#30363d' }}>
                <p className="font-medium mb-3 text-sm" style={{ color: '#e6edf3' }}>
                  <span className="text-xs px-1.5 py-0.5 rounded mr-2" style={{ background: '#1f6feb20', color: '#1f6feb' }}>{i + 1}</span>
                  {q.text}
                </p>
                <div className="space-y-2">
                  {q.options.map((opt, j) => {
                    ...
                    if (submitted) {
                      ...
                      else if (isSelected) { bgColor = '#f8514920'; borderColor = '#f85149'; textColor = '#f85149' }
                    ...
                    return (
                      <button key={j} onClick={() => !submitted && setAnswers(prev => ({ ...prev, [q.id]: j }))} disabled={submitted}
                        className="w-full text-left p-3 rounded-lg border text-sm flex items-center gap-2 transition-all"
            ...
            {!submitted ? (
              <button onClick={handleSubmitQuestions} disabled={Object.keys(answers).length < report.questions.length}
                className="w-full p-3 rounded-xl font-medium transition-all hover:opacity-90 disabled:opacity-50"
                style={{ background: '#238636', color: 'white' }}
              >
                Enviar Respostas ({Object.keys(answers).length}/{report.questions.length})
              </button>
            ) : (
              <button onClick={handleFinish} className="w-full flex items-center justify-center gap-2 p-3 rounded-xl font-medium transition-all hover:opacity-90" style={{ background: '#1f6feb', color: 'white' }}>
                Ver Feedback Final <ArrowRight size={16} />
              </button>
            )}
          </div>
        )}
            ...
            <div className="p-6 rounded-xl border" style={{ background: '#161b22', borderColor: '#30363d' }}>
              <div className="space-y-1">
                {report.analysis.feedback.split('\n').map((line, i) => {
                  if (line.startsWith('## ')) return <h2 key={i} className="text-xl font-bold mt-0 mb-3" style={{ color: '#e6edf3' }}>{line.replace('## ', '')}</h2>
                  if (line.startsWith('### ')) return <h3 key={i} className="text-base font-semibold mt-4 mb-2" style={{ color: '#1f6feb' }}>{line.replace('### ', '')}</h3>
                  if (line.startsWith('**')) { const match = line.match(/\*\*(.+?)\*\*(.*)/); return match ? <p key={i} className="text-sm my-1"><strong style={{ color: '#e6edf3' }}>{match[1]}</strong><span style={{ color: '#8b949e' }}>{match[2]}</span></p> : null }
                  if (line.match(/^[✅❌💪🌟👍]/u)) return <p key={i} className="text-sm my-1" style={{ color: '#e6edf3' }}>{line}</p>
                  if (line.startsWith('- ')) return <div key={i} className="flex items-start gap-2 text-sm my-1"><span style={{ color: '#238636' }}>•</span><span style={{ color: '#8b949e' }}>{line.replace('- ', '')}</span></div>
                  if (line.trim()) return <p key={i} className="text-sm" style={{ color: '#8b949e' }}>{line}</p>
                  return <br key={i} />
                })}
