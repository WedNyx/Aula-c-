...
export default function TeacherCalendar() {
  ...
  const handleSaveEntry = () => {
    ...
    if (existing) {
      updateCalendarEntry(existing.id, { contentName: editContent, city: editCity })
    } else {
      addCalendarEntry(selectedDate, editContent, editCity)
    }
    setEditMode(false)
  }
          ...
          <div className="mb-6 p-4 rounded-xl border flex items-center gap-4" style={{ background: '#1a2e1a', borderColor: '#238636' }}>
            ...
            <div>
              ...
              <p className="text-xs flex items-center gap-1" style={{ color: '#8b949e' }}>
                <MapPin size={10} /> {todayEntry.city}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 rounded-xl border p-4" style={{ background: '#161b22', borderColor: '#30363d' }}>
            <div className="flex items-center justify-between mb-4">
              <button onClick={prevMonth} className="p-1 rounded hover:bg-gray-700" style={{ color: '#8b949e' }}>
                <ChevronLeft size={18} />
              </button>
              <h2 className="font-bold" style={{ color: '#e6edf3' }}>
                {MONTHS[viewMonth]} {viewYear}
              </h2>
              <button onClick={nextMonth} className="p-1 rounded hover:bg-gray-700" style={{ color: '#8b949e' }}>
            ...
            <div className="grid grid-cols-7 mb-2">
              {DAYS.map(d => (
                <div key={d} className="text-center text-xs py-1 font-semibold" style={{ color: '#8b949e' }}>{d}</div>
              ))}
            </div>
              ...
              {cells.map((day, i) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: isSelected ? '#1f6feb' : isToday ? '#23863640' : 'transparent',
                      color: isSelected ? 'white' : isToday ? '#3fb950' : '#e6edf3',
                      border: isToday && !isSelected ? '1px solid #238636' : '1px solid transparent',
                    }}
                  >
                    {day}
                    {hasEntry && (
                      <div className="w-1 h-1 rounded-full mt-0.5" style={{ background: isSelected ? 'white' : '#d29922' }} />
                    )}
                  </button>
                )
              })}
          ...
          <div className="rounded-xl border p-4" style={{ background: '#161b22', borderColor: '#30363d' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-sm" style={{ color: '#e6edf3' }}>
                {new Date(selectedDate + 'T12:00').toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' })}
              </h3>
              {!editMode && (
                <button onClick={() => { setEditMode(true); setEditContent(selectedEntry?.contentName || ''); setEditCity(selectedEntry?.city || 'Plano Piloto (Brasília)') }} style={{ color: '#8b949e' }}>
                  {selectedEntry ? <Pencil size={14} /> : <Plus size={14} />}
                </button>
              )}
            </div>
            ...
            {editMode ? (
              ...
              <div className="space-y-3">
                <div className="p-3 rounded-lg" style={{ background: '#0d1117' }}>
                  <p className="text-xs" style={{ color: '#8b949e' }}>Conteúdo:</p>
                  <p className="font-medium mt-1" style={{ color: '#e6edf3' }}>{selectedEntry.contentName}</p>
                </div>
                <div className="p-3 rounded-lg flex items-center gap-2" style={{ background: '#0d1117' }}>
                  <MapPin size={14} color="#8b949e" />
                  <div>
                    <p className="text-xs" style={{ color: '#8b949e' }}>Cidade:</p>
                    <p className="text-sm" style={{ color: '#e6edf3' }}>{selectedEntry.city}</p>
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="mt-4 border-t pt-4" style={{ borderColor: '#30363d' }}>
              ...
              <div className="space-y-1">
                {CONTENT_SUGGESTIONS.slice(0, 8).map(s => (
                  <button
                    key={s}
                    onClick={() => { setEditContent(s); setEditMode(true) }}
                    className="w-full text-left text-xs p-2 rounded-lg hover:bg-gray-700 transition-colors"
                    style={{ color: '#8b949e' }}
                  >
                    {s}
                  </button>
