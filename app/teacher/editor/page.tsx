...
export default function TeacherEditor() {
  ...
  useEffect(() => {
    if (teacherFiles.length === 0) {
      const file = addTeacherFile('Exemplo.cs')
      setSelectedFileId(file.id)
    } else {
      setSelectedFileId(teacherFiles[0].id)
    }
  }, [])
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const lines = (data.output || 'Sem saída.').split('\n')
      setTerminalOutput(prev => [...prev, ...lines])
    } catch {
      setTerminalOutput(prev => [...prev, '❌ Erro ao executar.'])
    }
    setIsRunning(false)
  }
  ...
  const handleStartRename = (id: string, name: string) => {
    setRenamingId(id)
    setRenameValue(name.replace('.cs', ''))
  }

  const handleRename = (id: string) => {
    if (!renameValue.trim()) return
    renameTeacherFile(id, renameValue.trim() + '.cs')
    setRenamingId(null)
  }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-52 flex-shrink-0 border-r flex flex-col" style={{ background: '#161b22', borderColor: '#30363d' }}>
          <div className="p-2 border-b flex items-center justify-between" style={{ borderColor: '#30363d' }}>
            ...
            <button onClick={() => setShowNewFile(true)} className="p-1 rounded hover:bg-gray-700" style={{ color: '#8b949e' }}>
              <FilePlus size={14} />
            </button>
          </div>
          {showNewFile && (
              ...
              >
                ...
                {renamingId === file.id ? (
                  <input
                    ...
                    style={{ color: '#e6edf3', borderColor: '#bc8cff' }}
                  />
                ) : (
                  <span className="flex-1 text-xs truncate" style={{ color: '#e6edf3' }}>{file.name}</span>
                )}
                <div className="hidden group-hover:flex items-center gap-0.5">
                  <button onClick={e => { e.stopPropagation(); handleStartRename(file.id, file.name) }} className="p-0.5 rounded" style={{ color: '#8b949e' }}>
                    <Pencil size={10} />
                  </button>
                  {teacherFiles.length > 1 && (
                    <button onClick={e => { e.stopPropagation(); handleDelete(file.id) }} className="p-0.5 rounded" style={{ color: '#f85149' }}>
                      <Trash2 size={10} />
                    </button>
                  )}
                </div>
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="h-36 flex-shrink-0 border-t flex flex-col" style={{ background: '#0d1117', borderColor: '#30363d' }}>
              <div className="flex items-center justify-between px-3 py-1 border-b" style={{ background: '#161b22', borderColor: '#30363d' }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs" style={{ color: '#8b949e' }}>Terminal</span>
                </div>
          ...
          <div className="w-64 flex-shrink-0 border-l flex flex-col" style={{ background: '#161b22', borderColor: '#30363d' }}>
            <div className="p-3 border-b flex items-center gap-2" style={{ borderColor: '#30363d' }}>
              <Bot size={16} color="#bc8cff" />
              <span className="text-sm font-semibold" style={{ color: '#e6edf3' }}>Análise</span>
            </div>
