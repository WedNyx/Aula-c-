...
export default function TeacherDashboard() {
  ...
  const getStudentStatus = (studentId: string) => {
    return activeStudentStatuses[studentId] || 'offline'
  }

  const getStudentPerformance = (studentId: string) => {
    const session = sessions.find(s => s.studentId === studentId && s.date === today)
    if (!session) return null
    return session
  }

  const presentCount = todayAttendance.filter(a => a.status === 'presente').length
  const doingWork = filtered.filter(s => {
    const status = getStudentStatus(s.id)
    return ['editor', 'resumo', 'atividades', 'concluido'].includes(status)
  }).length
  const needHelp = filtered.filter(s => {
    const session = getStudentPerformance(s.id)
    return session && session.status === 'coding'
  }).length
    ...
    <div className="min-h-screen" style={{ background: '#0d1117' }}>
      <div className="border-b px-6 py-3 flex items-center justify-between" style={{ background: '#161b22', borderColor: '#30363d' }}>
        ...
        <div className="flex gap-2 items-center">
          <span className="text-xs px-2 py-0.5 rounded" style={{ background: '#23863620', color: '#3fb950' }}>
            ● Ao vivo
          </span>
          {(['all', 'matutino', 'vespertino'] as const).map(c => (
            <button
              ...
              style={{
                background: filterClass === c ? '#1f6feb' : '#21262d',
              ...
              .map(student => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: '#161b22',
                      borderColor: isInactive ? '#30363d' : (statusInfo?.color || '#30363d') + '60',
                    }}
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span
                        ...
                        style={{
                          background: isInactive ? '#21262d' : (statusInfo?.color || '#30363d') + '20',
                          color: isInactive ? '#8b949e' : statusInfo?.color || '#8b949e',
                        }}
                      >
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span style={{ color: '#e6edf3' }}>
                            {session.status === 'done' ? '✅ Concluído' :
                             session.status === 'questions' ? '📝 Atividades' :
                             session.status === 'summary' ? '📖 Resumo' :
                             '💻 Codando'}
                          </span>
                        </div>
                      )}
                    ...
                    {/* Warning if inactive but present */}
                    {att?.status === 'presente' && isInactive && (
                      <div className="mt-2 p-2 rounded text-xs flex items-center gap-1" style={{ background: '#d2992210', color: '#d29922' }}>
                        ...
                        Aluno presente mas não codou hoje
                      </div>
                    )}
